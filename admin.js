...
async function delData(id) {
   let res=await fetch(`http://localhost:5000/data/${id}`,{"method":"DELETE"})
   //http://localhost:5000/data
...
async function saveData(){
    ...
    let URL=studentId?`http://localhost:5000/data/${studentId}`:"http://localhost:5000/data"
...
async function editData(id){
    ...
    let res=await fetch(`http://localhost:5000/data/${id}`)
   let data=await res.json();
